...
File Description: Student functions to enroll, view and drop courses
================================================================================================================================
*/
...
#define course_path "/home/kalyani/linux/course_registration_system/datafiles/course.txt"
#define enrolled_course_path "/home/kalyani/linux/course_registration_system/datafiles/enrolled_course.txt"
...
int view_all_courses(struct Student loggedin_student, int client_socket){
    ...
    int fd_course = open(course_path,O_RDWR);
	if(fd_course == -1){
		perror("File error");
	}
    ...
    while (read(fd_course, &course, sizeof(struct Course)) > 0) {
        ...
        printf("\n");
        bzero(course_entries, sizeof(course_entries));
...
int enroll_course(struct Student loggedin_student, int client_socket){
    ...
    int num = atoi(string_response);
    
    while (read(fd_course, &course, sizeof(struct Course)) > 0) {
    	if(num == course.id[0] && course.avail_seats <= 0) {
    		send(client_socket, "No seats available\n", strlen("No seats available\n"), 0);
    		lock.l_type=F_UNLCK;
		fcntl(fd_course,F_SETLKW,&lock);
    		close(fd_course);
    ...
    //update student 
    int fd_student1 = open(Student_Path,O_RDWR);
    fd_course = open(course_path, O_RDONLY);
    ...
    while (read(fd_student1, &student, sizeof(struct Student)) > 0) {
        ...
        printf("\n");
        if(loggedin_student.roll == student.roll && student.enrolled < 2){
    ...
    int fd_student = open(enrolled_course_path,O_RDWR);
    ...
    fd_course = open(course_path,O_RDWR);
	if(fd_course == -1){
		perror("File error");
	}
    else{
...
int enrolled_course(struct Student loggedin_student, int client_socket){
    ...
    int fd_course = open("enrolled_course_path",O_RDWR);
	if(fd_course == -1){
		perror("File error");
	}
